...
class Harddisk:
	...
	def createPartition(self):
		...
		if disk_size > 2.2 * 1000 * 1000: # if 2.2 TB
			setAlign = ""
			cmd = 'parted --align optimal %s --script mklabel gpt mkpart disk ext2 0%% 100%%' % ( self.disk_path )
	...
	def mount(self):
		...
		if fileExists("/usr/lib/enigma2/python/Plugins/SystemPlugins/DeviceManager2/plugin.pyo"):
                        lines.append(self.partitionPath("1") + "   /media/hdd  auto  defaults  0 0")
		...
		for line in lines:
			parts = line.strip().split(" ")
                        ...
                        try:                                                                                
                                if MajorMinor(real_path) == MajorMinor(self.partitionPath(real_path[-1])):
                                        if fileExists("/usr/lib/enigma2/python/Plugins/SystemPlugins/DeviceManager2/plugin.pyo"):
                                                  cmd = "mount -t ext3 " + self.partitionPath("1") + " /media/hdd"
                                        else:
