...
# process the new timer
		...
		if ext_timer != 1:
			self.newtimer = ext_timer
		if (self.newtimer is not None) and (not self.newtimer.disabled) and (not self.newtimer.justremind):
...
# now process existing timers
		idx = 0
		for timer in self.timerlist:
			if (timer != self.newtimer) and (not timer.disabled) and (not timer.justremind):
				if timer.repeated:
