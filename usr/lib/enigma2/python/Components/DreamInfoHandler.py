# -*- coding: iso-8859-1 -*-
import xml.sax
from Tools.Directories import crawlDirectory, resolveFilename, SCOPE_CONFIG, SCOPE_SKIN, SCOPE_METADIR, copyfile, copytree, fileExists
...
class InfoHandler(xml.sax.ContentHandler):
	def __init__(self, prerequisiteMet, directory):
		...
		self.prerequisites = {}
		self.elements = []
		self.validFileTypes = ["skin", "config", "bouquetroot", "services", "favourites", "package"]
...
class DreamInfoHandler:
	...
	def installPackage(self, index):
		...
		self.attributeNames = ["skin", "config", "bouquetroot", "favourites", "package", "services"]
	...
	def installNext(self, *args, **kwargs):
		...
		if self.currentAttributeIndex >= len(self.attributeNames): # end of package reached
			...
			if len(self.bouquet_list_tv) or len(self.bouquet_list_radio):
				self.writeFavouritesList()
		...
		elif currentAttribute == "bouquetroot":
			favourite = attributes["bouquetroot"][self.currentIndex]
			self.installFavourites(favourite["directory"], favourite["name"], is_root_bouquet = True)
	...
	def mergeFavourites2Root(self, name):
		...
		if root_type == "radio":
			new_favourite = '#SERVICE 1:7:2:0:0:0:0:0:0:0:FROM BOUQUET \"%s\" ORDER BY bouquet' % name
			self.bouquet_list_radio.append(new_favourite)	
		else:
			new_favourite = '#SERVICE 1:7:1:0:0:0:0:0:0:0:FROM BOUQUET \"%s\" ORDER BY bouquet' % name
			self.bouquet_list_tv.append(new_favourite)
	...
	def installFavourites(self, directory, name, is_root_bouquet = False):
		...
		if self.blocking:
			...
			if self.console.execute("cp %s %s" % ((directory + name), resolveFilename(SCOPE_CONFIG))):
				print "execute failed"
				if not is_root_bouquet:
					self.mergeFavourites2Root(name)
				self.installNext()
			else:
				if not is_root_bouquet:
					self.mergeFavourites2Root(name)
