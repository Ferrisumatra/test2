...
class TimerEntry(Screen, ConfigListScreen):
	...
	def createConfig(self):
			...
			zapbeforerecord = self.timer.zapbeforerecord
			justremind = self.timer.justremind
			...
			if zapbeforerecord:
				...
				default_type = justplay
			self.timerentry_justplay = ConfigSelection(choices = [("zap", _("zap")), ("record", _("record")), ("zap_and_record", _("zap and record")), ("reminder", _("reminder"))], default = {0: "record", 1: "zap", 2: "zap_and_record", 3: "reminder"}[default_type])
	...
	def createSetup(self, widget):
		...
		if self.timerentry_justplay.value in ("zap", "reminder"):
			self.list.append(self.entryShowEndTime)
		self.entryEndTime = getConfigListEntry(_("EndTime"), self.timerentry_endtime)
		if self.timerentry_justplay.value not in ("zap", "reminder") or self.timerentry_showendtime.value:
		...
		if self.timerentry_justplay.value not in ("zap", "reminder"):
	...
	def keyGo(self, result = None):
		...
		self.timer.zapbeforerecord = self.timerentry_justplay.value == "zap_and_record"
		self.timer.justremind = self.timerentry_justplay.value == "reminder"
