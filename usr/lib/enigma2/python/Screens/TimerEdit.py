...
class TimerEditList(Screen, ProtectedScreen):
	...
	def isProtected(self):
		return config.ParentalControl.setuppinactive.value and config.ParentalControl.config_sections.timer_list.value
	...
	def fillTimerList(self):
		...
		if self.show_only_running:
			list.extend([(timer, False) for timer in self.session.nav.RecordTimer.timer_list if timer.state == 2])
		else:
	...
	def openEventView(self):
		...
		if event:
			#FIXME import EventViewSimple here to prevent recursive import in EventView which imports TimerSanityConflict
