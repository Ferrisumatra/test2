...
class ChannelContextMenu(Screen):
		...
		self["actions"] = ActionMap(["OkCancelActions", "ColorActions", "NumberActions"],
			{
				...
				"yellow": self.showServiceInSplitScreen,
				"green": self.showServiceInAudioZap
			})
		menu = [ ]
			...
			if not inBouquetRootList:
				...
				if isPlayable:
					if config.servicelist.startupservice.value != self.csel.getCurrentSelection().toString():
						append_when_current_valid(current, menu, (_("use as startup service"), self.useStartupService), level = 0)
					if config.servicelist.startupservice.value != "":
						append_when_current_valid(current, menu, (_("deactivate startup service"), self.deactivateStartupService), level = 0)
				...
				if config.misc.allow_service_delete.value == True:
					if (not inBouquet) and isPlayable:
						append_when_current_valid(current, menu, (_("remove entry"), self.removeCurrentRootService), level = 0)
						append_when_current_valid(current, menu, (_("remove all services without bouquet entry"), self.removeAllRootService), level = 0)
				...
				if isPlayable and SystemInfo.get("NumVideoDecoders", 1) > 1:
					append_when_current_valid(current, menu, (_("Activate Audio Zap"), self.showServiceInAudioZap), level = 0, key = "green")
					append_when_current_valid(current, menu, (_("Activate Split Screen"), self.showServiceInSplitScreen), level = 0, key = "yellow")
	...
	def getPiPServicePath(self, newservice):
		if self.csel.mode == MODE_TV:
			base_root = '1:7:1:0:0:0:0:0:0:0:FROM BOUQUET "bouquets.tv" ORDER BY bouquet'
		else:
			base_root = '1:7:1:0:0:0:0:0:0:0:FROM BOUQUET "bouquets.radio" ORDER BY bouquet'
		base_root = eServiceReference(base_root)
		root = self.csel.getRoot()
		return [base_root, root, newservice]
	...
	def showServiceInPiP(self):
		...
		servicepath = self.getPiPServicePath(newservice)
		from Screens.InfoBar import InfoBar
		InfoBar.instance.setPiP_globals()
		InfoBar.instance.showPiP()
		self.activateNewPiPService(newservice, servicepath)

	def showServiceInAudioZap(self):
		if not self.pipAvailable:
			return
		if self.session.pipshown:
			del self.session.pip
		...
		from Screens.InfoBar import InfoBar
		InfoBar.instance.setPiP_globals()
		InfoBar.instance.showAudioZap()
		self.activateNewPiPService(newservice, servicepath)

	def showServiceInSplitScreen(self):
		if not self.pipAvailable:
			return
		if self.session.pipshown:
			del self.session.pip
		...
		from Screens.InfoBar import InfoBar
		InfoBar.instance.setPiP_globals()
		InfoBar.instance.showSplitScreen()
		self.activateNewPiPService(newservice, servicepath)

	def activateNewPiPService(self, newservice, servicepath):
		if self.session.pip.playService(newservice):
			if config.usage.zap_pip.value and not self.session.is_splitscreen and not self.session.is_audiozap and servicepath:
				self.csel.setCurrentServicePath(servicepath, zap = False)
			else:
	...
	def activateServiceInSplitScreen(self):
		if not self.pipAvailable:
			return
		if self.session.pipshown:
			del self.session.pip
		...
		if self.session.pip.playService(newservice):
			...
			self.session.pipshown = False
			del self.session.pip
			self.session.openWithCallback(self.close, MessageBox, _("Could not open Split Screen"), MessageBox.TYPE_ERROR)
	...
	def removeCurrentRootService(self):
		self.csel.removeCurrentRootService()
		self.close()

	def removeAllRootService(self):
		self.csel.removeAllRootService()
		self.close()
...
class ChannelSelectionEPG:
	...
	def showEPGList(self):
		...
		if ref and root:
			...
			self.savedRoot = root
			self.session.openWithCallback(self.SingleServiceEPGClosed, EPGSelection, ref, zapFunc = self.zapToService, serviceChangeCB=self.changeServiceCB, switchBouquet = self.switchBouquet)
	
	def zapToService(self, service, check_correct_bouquet = False):
		self.servicelist.setCurrent(service)
		self.savedRoot = self.getRoot()
		self.savedService = service
		self.zapped = True
		self.zap()

	def switchBouquet(self, service, direction, epg):
		if direction > 0:
			self.nextBouquet()
		else:
			self.prevBouquet()
	...
	def SingleServiceEPGClosed(self, ret=False):
		self.pathUp()
		self.enterPath(self.savedRoot)
		self.setCurrentSelection(self.savedService)
		if self.zapped and not config.usage.servicelist_preview_mode.value:
			self.close()
...
#  multiple marked entry stuff ( edit mode, later multiepg selection )
	...
	def removeCurrentRootService(self):
		...
		try:
			...
			pass
		db.saveServicelist("/etc/enigma2/lamedb")
		self.servicelist.removeCurrent()
		self.servicelist.moveUp()
	...
	def removeAllRootService(self):
		root=self.getRoot()
		serviceHandler = eServiceCenter.getInstance()
		allservices = serviceHandler.list(root)
		fullservicelist = allservices and allservices.getContent("RN", True)
		nodelete = []
		bouquets = self.getBouquetList()
		for bouquet in bouquets:
			servicesb = serviceHandler.list(bouquet[1])
			bouquetlist = servicesb and servicesb.getContent("S", True)
			for serviceinbouquet in bouquetlist:
				nodelete.append(serviceinbouquet)
		if len(nodelete):
			db = eDVBDB.getInstance()
			for service in fullservicelist:
				servicestr = service[0].toString()
				serviceswig = service[0]
				servicename = service[1]
				if servicestr in nodelete:
					print "service %s in user bouquet => no deletion" % servicename
				else:
					self.servicelist.setCurrent(serviceswig)
					self.servicelist.removeCurrent()
					db.removeService(serviceswig)
			from os import unlink
			from Tools.Directories import resolveFilename, SCOPE_CONFIG
			try:
				unlink(resolveFilename(SCOPE_CONFIG) + "/lamedb");
...
class ChannelSelectionBase(Screen):
			...
			if pos != -1:
				...
				if Len > 0:
					...
					if end_ref is not None:
						...
						titles = _("Channel Selection") + " " +titleStr + " " + nameStr
						self["etitle"].setText(titles)
		else:	
		#	
			if pos != -1:
				...
				if Len > 0:
					...
					if end_ref is not None:
						...
						nameStr = self.getServiceName(end_ref)
						titleStr += nameStr
					titleStr = titleStr.replace("Channel Selection", _("Channel Selection"))
					titleStr = titleStr.replace("Favourites", _("Favourites"))
	...
	def showAllServices(self):
		if not self.pathChangeDisabled:
			cur_sel = self.getCurrentSelection()
			...
			if not self.preEnterPath(refstr):
				...
				if currentRoot is None or currentRoot != ref:
					self.clearPath()
					self.enterPath(ref)
			if not cur_sel.flags & eServiceReference.isDirectory:
				if cur_sel.type == 1:
					self.setCurrentSelection(cur_sel)
...
config.servicelist.startup_path = ConfigText()
...
class ChannelSelection(ChannelSelectionBase, ChannelSelectionEdit, ChannelSelectionEPG, SelectionEventInfo):
	def __init__(self, session):
		...
		if config.servicelist.startupservice.value and config.servicelist.startup_path.value:
	...
	def channelSelected(self):
		...
		if self.movemode:
			...
			if not root or not (root.flags & eServiceReference.isGroup):
				__close = True
				if config.usage.servicelist_preview_mode.value:
					if config.ParentalControl.configured.value:
						from Components.ParentalControl import parentalControl
						if parentalControl.getProtectionLevel(ref.toCompareString()) != -1:
							__close = False
					if __close:
						ref_play = self.session.nav.getCurrentlyPlayingServiceReference()
						alternative = None
						if ref.flags & eServiceReference.isGroup:
							alternative_list = ServiceReference(ref).list()
							alternative = alternative_list and alternative_list.getNext()
						if ref_play is None or (ref_play != ref and (alternative is None or alternative != ref_play)):
							self.session.nav.playService(ref)
							self.service_list_preview = True
							__close = False
				if __close:
					self.zap()
					self.asciiOff()
	...
	def zap(self):
		self.revertMode=None
		if not self.session.pip_zap_main and self.session.pipshown:
			...
			nref = self.getCurrentSelection()
			if ref is None or ref != nref or (config.usage.servicelist_preview_mode.value and ref == nref):
				self.new_service_played = True
	...
	def historyIndex(self, idx):
		hlen = len(self.history)
		if hlen > 1 and idx <= (hlen + 1):
			self.history_pos = idx
			self.setHistoryPath()

	def setHistoryPath(self, zap = True):
	...
	def setCurrentServicePath(self, path, zap = True):
		if self.history:
			self.history[self.history_pos] = path
		else:
			self.history.append(path)
		self.setHistoryPath(zap)
	...
	def cancel(self):
		if config.usage.servicelist_preview_mode.value:
			...
			if ref_play != lastservice and lastservice.valid():
	...
	def closePiG(self):
		if self.session.pipshown and self.session.is_pig:
			from Screens.InfoBar import InfoBar
			InfoBar.instance.showPiP()
			InfoBar.instance.setPiP_globals()
