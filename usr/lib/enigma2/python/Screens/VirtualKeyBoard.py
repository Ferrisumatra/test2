...
class VirtualKeyBoardList(MenuList):
	def __init__(self, list, enableWrapAround=False):
		...
		font, size = skin.parameters.get("VirtualKeyboardFont", ('Regular', 28))
		self.l.setFont(0, gFont(font, size))
		self.l.setItemHeight(int(skin.parameters.get("VirtualKeyboard", (45,))[0]))
...
def VirtualKeyBoardEntryComponent(keys, selectedKey,shiftMode=False):
	...
	height = int(skin.parameters.get("VirtualKeyboard", (45,))[0])
	...
	for key in keys:
		...
		if key == "EXIT":
			...
			res.append(MultiContentEntryPixmapAlphaTest(pos=(x, 0), size=(width, height), png=key_left))
		elif key == "RIGHT":
			width = key_right.size().width()
			res.append(MultiContentEntryPixmapAlphaTest(pos=(x, 0), size=(width, height), png=key_right))
		else:
